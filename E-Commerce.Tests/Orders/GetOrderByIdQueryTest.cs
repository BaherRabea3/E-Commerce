...
namespace E_Commerce.Tests.Orders
{
    public class GetOrderByIdQueryTest
    {
        ...
        [Fact]
        public async Task Handler_Should_ReturnFailure_WhenOrderIsNotFound()
        {
            //Arrange
            var command = new GetOrderByIdQuery(5,1);
            //Act
        ...
        {
            //Arrange
            var command = new GetOrderByIdQuery(1, 1);
            //Act
            var result = await _handler.Handle(command, CancellationToken.None);
            //Assert
            result.IsSuccess.Should().BeTrue();
            result.Value.Should().BeOfType<OrderDetailResponseDto>();
        }
